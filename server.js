...
async function writeDB(data) {
  ...
  try {
    // make sure the data directory exists (prevents local write errors)
    await fs.mkdir(path.dirname(DB_PATH), { recursive: true });
  } catch (e) {
    // ignore mkdir errors (permission etc.) and let write fail if needed
  }
...
// NEW: return full reminders for a specific user (frontend calls this)
      ...
      for (const rem of user.reminders.slice()) {
        try {
          if (shouldFireReminder(rem, nowUtcMs)) {
            // use absolute frontend URL so notifications open the right page
            const payload = { title: 'Hydrate — Reminder', body: `Time: ${rem.time} — Drink water 💧`, url: 'https://rahulmm22.github.io/hydrate-frontend' };
            try {
